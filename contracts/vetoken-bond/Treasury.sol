...
contract Treasury is Ownable {
    ...
    event Simp(address indexed token, uint256 amount, address niceGuy);
    ...
    event tokenValueSet(address token, uint256 value);
    ...
    uint256 public immutable newBlocksNeededForQueue;

    address[] public reserveTokens; // Push only, beware false-positives.
    mapping(address => bool) public isReserveToken;
    mapping(address => uint256) public reserveTokenQueue; // Delays changes to mapping.
    mapping(address => uint256) public nativeFloorPrice; // With 9 decimals of precision
    ...
    address public sVETOKEN;
    uint256 public sVETOKENQueue; // Delays change to sVETOKEN address
    ...
    constructor(
        ...
        uint256 _CVXFloor,
        uint256 _CRVFloor,
        uint256 _newBlocksNeededForQueue
    ) {
    ...
    /**
        @notice allow approved address to deposit an asset for VETOKEN
        @param _amount uint
    ...
    function deposit(
        ...
        // mint VETOKEN needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(VETOKEN).mint(msg.sender, send_);
    ...
    function simp(uint256 _amount, address _token) external {
        require(isReserveToken[_token] || isLiquidityToken[_token], "Not accepted");
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = valueOf(_token, _amount);
        totalReserves = totalReserves.add(value);
    ...
    /**
        @notice allow approved address to burn VETOKEN for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IVETOKENERC20(VETOKEN).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sVETOKEN).balanceOf(msg.sender); // Can only borrow against sVETOKEN held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with VETOKEN
        @param _amount uint
     */
    function repayDebtWithVETOKEN(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        IVETOKENERC20(VETOKEN).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, VETOKEN, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(VETOKEN).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(VETOKEN).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns VETOKEN valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf(address _token, uint256 _amount) public view returns (uint256 value_) {
        if (isReserveToken[_token]) {
            // convert amount to match VETOKEN decimals and divide by number of tokens for 1 VETOKEN
            value_ = _amount
                .mul(10**IERC20(VETOKEN).decimals())
                .div(10**IERC20(_token).decimals())
                .mul(10**9)
                .div(nativeFloorPrice[_token]);
    ...
    /**
        @notice sets VETOKEN valuation of asset
        @param _token address
        @param _value uint
     */
    function setFloor(address _token, uint256 _value) external onlyManager {
    ...
    function queue(MANAGING _managing, address _address) external onlyManager returns (bool) {
        ...
        } else if (_managing == MANAGING.SVETOKEN) {
            // 9
            sVETOKENQueue = block.number.add(blocksNeededForQueue);
        } else return false;
