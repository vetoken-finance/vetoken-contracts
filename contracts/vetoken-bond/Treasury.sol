...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SVETOKEN }
    ...
    uint public sVETOKENQueue; // Delays change to sVETOKEN address
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sVETOKEN ).balanceOf( msg.sender ); // Can only borrow against sVETOKEN held
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithVETOKEN( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( VETOKEN ).decimals() ).div( 10 ** IERC20( _token ).decimals() ).mul(10 ** 9).div(nativeFloorPrice[_token]);
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SVETOKEN ) { // 9
